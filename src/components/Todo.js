...
class Todo extends React.Component {
  ...
  onTextInputSave = name => {
    const { relay, todo } = this.props;

    this.setEditMode(false);

    RenameTodoMutation.commit(relay.environment, todo, name);
  };
  ...
  render() {
    const { complete, name } = this.props.todo;
    ...
    return (
      ...
      >
        <div className="view">
          ...
          />
          <label onDoubleClick={this.onLabelDoubleClick}>{name}</label>
          <button
            type="button"
            className="destroy"
        ...
        {!!this.state.isEditing && (
          <TodoTextInput
            className="edit"
            commitOnBlur
            initialValue={this.props.todo.name}

...
const RenameTodoMutation = mutationWithClientMutationId({
  ...
  outputFields: {
    todo: {
      ...
      resolve: async ({localTodoId}: Payload): Promise<Todo> =>
        await getTodoOrThrow(localTodoId),
    },
  },
  mutateAndGetPayload: async ({id, text}: Input): Promise<Payload> => {
    const localTodoId = fromGlobalId(id).id;
    await renameTodo(localTodoId, text);

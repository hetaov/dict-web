...
const MarkAllTodosMutation = mutationWithClientMutationId({
  ...
  outputFields: {
    changedTodos: {
      ...
      resolve: ({changedTodoIds}: Payload): $ReadOnlyArray<Promise<Todo>> => {
        return changedTodoIds.map(
          async (todoId: string): Promise<Todo> => await getTodoOrThrow(todoId),
        );
      },
    },
    user: {
  ...
  mutateAndGetPayload: async ({complete, userId}: Input): Promise<Payload> => {
    console.log(' mutateAndGetPayload, alld----> ');
